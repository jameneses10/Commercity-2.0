...
async function webhookAdmin({pedido_id,approved}){
 ...
 try{ await conn.beginTransaction();
  ...
  const details=await orderModel.getDetailsConn(conn,pedido_id);
  for(const d of details){ const [rows]=await conn.query(`SELECT p.id,p.stock,p.estado,${EFFECTIVE_PRICE_EXPR} AS precio_final FROM productos p WHERE p.id=? FOR UPDATE`,[d.producto_id]); const p=rows[0]; if(!p||p.estado!=='activo'||Number(p.stock)<Number(d.cantidad)) throw err('Producto no disponible para webhook.',409); ensureCurrentPrice(d,p); }
