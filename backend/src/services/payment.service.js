...
async function webhookAdmin(user,{pedido_id,approved}){
 ...
 try{ await conn.beginTransaction();
  ...
  if(!approved){ await paymentModel.create(conn,{pedido_id,metodo:'sandbox_webhook',referencia:reference('WH-REJ'),estado:'rechazado',mensaje:'Webhook sandbox rechazado.'}); await orderModel.updateStatus(conn,pedido_id,{estado_pago:'rechazado',estado_general:'creado'}); await logService.log(conn,{usuario_id:user.id,accion:'pago_rechazado',entidad:'pedidos',entidad_id:pedido_id,detalle:{metodo:'sandbox_webhook',estado_pago:'rechazado',estado_general:'creado'}}); await conn.commit(); return {estado:'rechazado'}; }
  const details=await orderModel.getDetailsConn(conn,pedido_id);
  const requiredByProduct=buildRequiredQuantityByProduct(details);
  ...
  await commissionService.createCommissions(conn,pedido_id,details); await shipmentModel.createMissingForPaidOrder(conn,pedido_id); await logService.log(conn,{usuario_id:user.id,accion:'pago_aprobado',entidad:'pedidos',entidad_id:pedido_id,detalle:{metodo:'sandbox_webhook',estado_pago:'pagado',estado_general:'procesando'}}); await conn.commit(); return {estado:'aprobado'};
