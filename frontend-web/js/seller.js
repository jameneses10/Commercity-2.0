...
function bindVisualActions(){ document.addEventListener('click',async e=>{ const v=e.target.closest('[data-product-visibility]'); if(v){ try{ await api.patch(`/products/${v.dataset.productVisibility}/visibility`,{estado:v.dataset.nextStatus}); v.textContent='Actualizado'; }catch(error){ v.textContent='Pendiente API'; console.warn(error.message); } } const s=e.target.closest('[data-shipment-status]'); if(s){ try{ await api.patch(`/shipments/${s.dataset.shipmentStatus}/status`,{estado:s.dataset.nextStatus}); s.textContent='Actualizado'; }catch(error){ s.textContent='AcciÃ³n preparada'; console.warn(error.message); } } const a=e.target.closest('[data-visual-action]'); if(a){ a.textContent=`${a.dataset.visualAction} preparado`; } }); }
