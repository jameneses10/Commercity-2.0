...
async function storesPage(){ let stores; try{ stores=await loadAllAdminStores(); }catch(error){ main().innerHTML=shell('Tiendas','cc-store-shopping-bag.svg','Tiendas','Listado administrativo real de tiendas.','')+empty('cc-store-shopping-bag.svg','No se pudo cargar el listado completo de tiendas.',error.message||'No fue posible cargar las tiendas.'); return; } main().innerHTML=shell('Tiendas','cc-store-shopping-bag.svg','Tiendas','Listado administrativo real de tiendas.','')+filters('stores',[['all','Todas'],['activa','Activas'],['pendiente','Pendientes'],['pausada','Suspendidas']],'Buscar tienda')+`<section class="cc-grid cols-2">${stores.map(s=>`<article class="cc-card" data-admin-item="stores" data-status="${esc(norm(s.estado))}" data-filter-text="${esc(`${s.id} ${s.nombre} ${s.estado} ${s.vendedor_nombre||''}`)}"><span class="cc-chip ${chipClass(s.estado)}">${esc(s.estado||'estado')}</span><h2>${esc(s.nombre)}</h2><p class="cc-muted">Vendedor: ${esc(s.vendedor_nombre||'No disponible')} · Productos: ${esc(s.total_productos||0)} · Reputación: ${esc(s.reputacion_promedio||0)}</p><div class="cc-card-actions-row"><a class="cc-btn outline" href="tienda.html?id=${esc(s.id)}">Ver tienda</a><button class="cc-btn" type="button" data-store-action="${esc(s.id)}" data-action="activate">Activar</button><button class="cc-btn secondary" type="button" data-store-action="${esc(s.id)}" data-action="pause">Suspender</button></div></article>`).join('')}</section>${stores.length?'':empty('cc-store-shopping-bag.svg','Sin tiendas reales.','El endpoint administrativo respondió vacío.')}`; bindFilters(main()); }
