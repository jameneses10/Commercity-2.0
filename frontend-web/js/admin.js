...
async function productsPage(){
  ...
  main().innerHTML=shell('Productos','cc-products-management.svg','Catálogo','Productos reales y reportes administrativos.')
    ...
    +`<section class="cc-table-wrap"><table class="cc-table"><thead><tr><th>Producto</th><th>Tienda</th><th>Categoría</th><th>Vendedor</th><th>Precio</th><th>Stock</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${products.map(p=>{
      ...
      return `<tr data-admin-item="products" data-status="${esc(canonicalState)}" data-filter-text="${esc(`${p.nombre} ${p.tienda_nombre} ${p.categoria_nombre} ${canonicalState} ${p.vendedor_nombre||''}${isReported?' reportado':''}`)}"><td><span class="cc-table-product"><img class="cc-icon" src="${esc(imageUrl(p.imagen_url))}" alt="">${esc(p.nombre)}</span></td><td>${esc(p.tienda_nombre||'')}</td><td>${esc(p.categoria_nombre||'')}</td><td>${esc(p.vendedor_nombre||'No disponible')}</td><td>${money(p.precio_final||p.precio||0)}</td><td>${esc(p.stock??0)}</td><td><span class="cc-chip ${chipClass(displayState)}">${esc(displayState)}</span></td><td><div class="cc-mini-actions"><a href="producto-detalle.html?id=${esc(id)}">Ver</a><button type="button" data-product-visibility="${esc(id)}" data-next-status="${displayState==='activo'?'oculto':'activo'}">${displayState==='activo'?'Ocultar':'Activar'}</button><button type="button" data-visual-action="Revisión de producto preparada">Revisar</button></div></td></tr>`;
